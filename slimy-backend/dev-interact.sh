...
dev_gen_wild() {
    ...
    mxpy contract call ${SC_OWN} --pem ${DEV} --gas-limit 5000000 --recall-nonce --function "createGenZeroSlime" --arguments ${HUNTER} --outfile="gen_wild.json" --send --proxy ${PROXY} --chain T > /dev/null
...
dev_catch() {
    ...
    mxpy contract call ${SC_OWN} --pem ${DEV} --gas-limit 5000000 --recall-nonce --function "catchSlime" --arguments ${HUNTER} ${WILD_ID} --outfile="catch_wild.json" --send --proxy ${PROXY} --chain T > /dev/null
...
dev_breed() {
    ...
    mxpy contract call ${SC_OWN} --pem ${DEV} --gas-limit 50000000 --recall-nonce --function "breedWith" --arguments ${MATRON_ID} ${SIRE_ID} --outfile="breed.json" --send --proxy ${PROXY} --chain T > /dev/null

    RESULT_TX=$(mxpy data parse --file breed.json --expression='data["emittedTransactionHash"]' | tail -n 1)
    sleep 5
    curl -s "${API}/transactions/${RESULT_TX}?fields=results" -H 'accept: application/json' > result.json
    OUTPUT=$(python3 parse_results.py)
    IFS='@' read -ra ADDR <<< "$OUTPUT"
