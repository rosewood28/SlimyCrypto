...
impl Random {
    ...
    pub fn quadded_u8(&mut self, min: u8, max: u8) -> u8 {
        let val = self.next_u8();

        let denom = ((u8::MAX - val) as u16).pow(2);
        let div = (u8::MAX as u16).pow(2);

        (denom as u32 * ((max - min + 1) as u32) / div as u32) as u8 + min
    }
